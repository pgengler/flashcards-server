...
class CardsControllerTest < ActionController::TestCase
	test "can create a new card" do
		assert_difference 'Card.count' do
			post :create, params: { card: { front: 'Front text', back: 'Back text' } }
		end
	end
	...
	test "returns correct HTTP status code when failing to create a new card" do
		post :create, params: { card: { front: '', back: '' } }
		assert_response :unprocessable_entity
	end

	test "can update a card" do
		put :update, params: { id: cards(:one), card: { front: 'New front text' } }
		cards(:one).reload
	...
	test "returns correct HTTP status code when failing to update a card" do
		put :update, params: { id: cards(:one), card: { front: '' } }
		assert_response :unprocessable_entity
	end

	test "returns correct HTTP status when trying to update a nonexistent card" do
		put :update, params: { id: 12345, card: { front: 'Front', back: 'Back' } }
		assert_response :not_found
	end

	test "can delete a card" do
		assert_difference('Card.count', -1) do
			delete :destroy, params: { id: cards(:one) }
		end
	end
	...
	test "JSON response for a single includes card sets" do
		get :show, params: { id: cards(:in_one_set) }
